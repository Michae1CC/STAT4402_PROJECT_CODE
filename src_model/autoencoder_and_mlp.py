# -*- coding: utf-8 -*-
"""autoencoder_and_mlp.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1IUury7F0oTdnVHnEWQvfwufLTTcQ81m5
"""
...
class Net(nn.Module):
  def __init__(self):
    ...
    self.hidden1 = nn.Sequential(nn.Linear(256, 200), # 256 = no. latent variables
                               nn.ReLU())
...
class autoencode(nn.Module):
  def __init__(self):
    ...
    self.encoder = nn.Sequential(nn.Linear(X.shape[1],256),nn.ReLU())
    self.decoder = nn.Linear(256,X.shape[1])
...
X_old  = X
X_org_graph_1 = X_old[0]
X_org_graph_2 = X_old[300]
X_org_graph_3 = X_old[1000]

X_ae,_ = trained_autoencoder(X)
# Get a point to graph IR before auto encoding
X_ae=X_ae.detach()

X_auto_graph_1 = X_ae[0]
X_auto_graph_2 = X_ae[300]
X_auto_graph_3 = X_ae[1000]
print(X_org_graph_1.shape)
print()
print(X_auto_graph_1.shape)

1
fig, ((ax1, ax2), (ax3, ax4), (ax5, ax6)) = plt.subplots(3, 2, figsize=(22,15))

ax1.plot(range(len(X_org_graph_1)), X_org_graph_1, color='red')
ax1.tick_params(
...
ax1.set_title('IR spec prior auto encoding')

ax2.plot(range(len(X_auto_graph_1)), X_auto_graph_1, color='red')
ax2.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
    bottom=False,      # ticks along the bottom edge are off
...
ax2.set_title('IR spec after auto encoding')

ax3.plot(range(len(X_org_graph_2)), X_org_graph_2, color='red')
ax3.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
    bottom=False,      # ticks along the bottom edge are off
...
ax3.set_title('IR spec prior auto encoding')

ax4.plot(range(len(X_auto_graph_2)), X_auto_graph_2, color='red')
ax4.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
    bottom=False,      # ticks along the bottom edge are off
    top=False,         # ticks along the top edge are off
    labelbottom=False)  # labels along the bottom edge are off
ax4.set_xlabel("Wavenumber 1/CM")
ax4.set_ylabel("Transmittance")
ax4.set_title('IR spec after auto encoding')

ax5.plot(range(len(X_org_graph_3)), X_org_graph_3, color='red')
ax5.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
    bottom=False,      # ticks along the bottom edge are off
    top=False,         # ticks along the top edge are off
    labelbottom=False)  # labels along the bottom edge are off
ax5.set_xlabel("Wavenumber 1/CM")
ax5.set_ylabel("Transmittance")
ax5.set_title('IR spec prior auto encoding')

ax6.plot(range(len(X_auto_graph_3)), X_auto_graph_3, color='red')
ax6.tick_params(
    axis='x',          # changes apply to the x-axis
    which='both',      # both major and minor ticks are affected
    bottom=False,      # ticks along the bottom edge are off
    top=False,         # ticks along the top edge are off
    labelbottom=False)  # labels along the bottom edge are off
ax6.set_xlabel("Wavenumber 1/CM")
ax6.set_ylabel("Transmittance")
ax6.set_title('IR spec after auto encoding')
...
def train(X = X,
  ...
  for epoch in range(epoch_num):
    ...
    print("Running loss: ", running_loss/batch_size)
...
_,X_test = trained_autoencoder(X_test)
