...
def example_load():
    ...
    x_data_path = os.path.join(
        '/content', 'drive', 'My Drive', 'STAT4402 Project', 'IR_MS_FUNCTIONAL_X.npy')
    y_data_path = os.path.join(
        '/content', 'drive', 'My Drive', 'STAT4402 Project', 'IR_MS_FUNCTIONAL_y.npy')
...
mapping = {tuple(vector): num for vector, num in zip(
    iter(unique_vecs), range(len(unique_vecs)))}
...
X_train, X_test, y_train, y_test = train_test_split(
    IR_MS_FUNCTIONAL_X, mapped_value, test_size=0.3, random_state=42)

steps = [('scaler', StandardScaler()), ('PCA', PCA(n_components=100)), ('RForest',
                                                                        RandomForestClassifier(max_depth=50, n_jobs=-1, criterion='entropy', verbose=1))]
pipe = Pipeline(steps, verbose=True)  # define Pipeline object
