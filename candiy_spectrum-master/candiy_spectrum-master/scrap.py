...
'''
    ...
    for cas_id in cas_ls:
        ...
        if response.text == '##TITLE=Spectrum not found.\n##END=\n':
            continue
        num_created += 1
        logging.info('Creating {} spectra for id: {}. Total spectra created {}'.format(
            params['Type'].lower(), cas_id, num_created))
...
'''
    ...
    with open(inchi_path, 'a') as file:
        ...
        for cas_id in cas_ls:
            ...
            num_created += 1
            logging.info('Creating InChi key for id: {}. Total keys created {}'.format(
                cas_id, num_created))
            content = '{}\t{}\n'.format(
                cas_id, response.content.decode("utf-8"))
            file.write(content)
...
assert os.path.isfile(
    args.cas_list), "No file named {} exists".format(args.cas_list)
...
cas_df = pd.read_csv(args.cas_list, sep='\t', names=[
                     'name', 'formula', 'cas'], header=0)
